"""Kubernetes manifest validator for Aeitron production deployments."""
...
def _container_issues(path: Path, resource: str, pod_spec: dict[str, Any]) -> list[K8sValidationIssue]:
    ...
    for container in pod_spec.get("containers", []):
        ...
        for env in container.get("env", []):
            ...
            is_control_flag = name_key in {"aeitron_allow_token_issue", "aeitron_auth_enabled", "aeitron_quota_enabled"}
...
def doc_requires_probe(resource: str) -> bool:
    return resource.startswith("Deployment/aeitron-api")
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Validate Aeitron Kubernetes manifests.")
