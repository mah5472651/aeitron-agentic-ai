...
from src.aeitron.learning.worker import CrawlerWorkerConfig, run_crawler_worker
...
class SupervisorConfig(StrictModel):
    ...
    object_store_uri: str = "s3://aeitron-datasets/pretraining"
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run supervised Aeitron data crawl cycles.")
    parser.add_argument("--sources", default=os.environ.get("AEITRON_DATA_SOURCES", "config/data_sources.ultimate.json"))
    parser.add_argument("--postgres-dsn", default=os.environ.get("AEITRON_DATABASE_URL", ""))
    parser.add_argument("--raw-output-dir", default=os.environ.get("AEITRON_RAW_OUTPUT_DIR", "artifacts/aeitron/data-engine/raw"))
    parser.add_argument("--clean-output-dir", default=os.environ.get("AEITRON_CLEAN_OUTPUT_DIR", "artifacts/aeitron/data-engine/clean"))
    parser.add_argument("--object-store-uri", default=os.environ.get("AEITRON_OBJECT_STORE_URI", "s3://aeitron-datasets/pretraining"))
    parser.add_argument("--heartbeat-path", default=os.environ.get("AEITRON_HEARTBEAT_PATH", "artifacts/aeitron/supervisor/heartbeat.json"))
    parser.add_argument("--status-path", default=os.environ.get("AEITRON_STATUS_PATH", "artifacts/aeitron/supervisor/status.json"))
    parser.add_argument("--worker-replicas", type=int, default=int(os.environ.get("AEITRON_WORKER_REPLICAS", "1")))
    parser.add_argument("--async-workers", type=int, default=int(os.environ.get("AEITRON_ASYNC_WORKERS", "16")))
    parser.add_argument("--batch-docs", type=int, default=int(os.environ.get("AEITRON_BATCH_DOCS", "10000")))
    parser.add_argument("--max-cycles", type=int, default=int(os.environ.get("AEITRON_SUPERVISOR_MAX_CYCLES", "100")))
    parser.add_argument("--sleep-seconds", type=float, default=float(os.environ.get("AEITRON_SUPERVISOR_SLEEP_SECONDS", "30")))
    parser.add_argument("--max-failures", type=int, default=int(os.environ.get("AEITRON_SUPERVISOR_MAX_FAILURES", "5")))
    return parser.parse_args()


def _config_from_args(args: argparse.Namespace) -> SupervisorConfig:
    if not args.postgres_dsn:
        raise ValueError("AEITRON_DATABASE_URL or --postgres-dsn is required")
