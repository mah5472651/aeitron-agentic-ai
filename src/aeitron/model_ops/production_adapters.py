"""Production adapter exports for Aeitron scratch checkpoints.

This module owns the bridge from the native Aeitron checkpoint format to external
    ...
    for layer_index in range(config.num_layers):
        ...
        for old_key, new_key in mapping.items():
            if old_key not in state:
                raise KeyError(f"missing Aeitron checkpoint tensor: {old_key}")
    ...
    *,
    ...
    conversion = {
        ...
        "required_validation": [
            ...
            "compare logits against native Aeitron on fixed token ids",
            "run vLLM smoke generation",
            "run TensorRT-LLM engine build and decode parity test",
        ],
    }
    (target / "aeitron_conversion_manifest.json").write_text(json.dumps(conversion, indent=2, sort_keys=True), encoding="utf-8")
    artifacts = [str(target / name) for name in ["config.json", "model.safetensors", "tokenizer.json", "tokenizer_config.json", "special_tokens_map.json", "aeitron_conversion_manifest.json"]]
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aeitron production adapter utilities.")
    sub = parser.add_subparsers(dest="command", required=True)
    hf = sub.add_parser("export-hf", help="Export native Aeitron checkpoint as HF Llama-compatible package.")
